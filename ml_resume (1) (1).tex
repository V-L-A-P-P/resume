...
\resumeItemListStart
  \resumeItem{Разрабатывал алгоритм оптимизации расписания рекламного вещания: учитывал бизнес-ограничения, овербукинг и обеспечивал равномерное распределение показов по слотам/времени.}
  \resumeItem{\textbf{Результат}: Разработал алгоритм генерации расписания; уменьшил долю ручных правок на 50\%.}
\resumeItemListEnd
...
\resumeItemListStart
  ...
  \resumeItem{Построил модели, отобрал лучшую и настроил гиперпараметры через Optuna с учётом сильного дисбаланса (1/250); финальная модель Catboost достигла \textbf{weighted accuracy = 0.87}.}
...
\resumeItem{Внедрил \textbf{LLM-based query rewriting} на базе instruction-tuned моделей для повышения релевантности поиска.}
...
\resumeItem{Разработал \textbf{граф связей между чанками} и интегрировал его в retrieval-этап для graph-augmented расширения кандидатов и reranking.}
\resumeItem{Реализовал \textbf{end-to-end пайплайн RAG} с \textbf{LLM answer generation}: ответы по найденным документам с указанием источников; тестирование и деплой (\textbf{PyTest, FastAPI, Docker}).}
